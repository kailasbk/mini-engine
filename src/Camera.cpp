...
void Camera::moveTo(const glm::vec3& destination) {
    m_position = destination;

    updateViewProjectionMatrix();
}

void Camera::lookAt(const glm::vec3& target, bool keepRoll) {
...
std::shared_ptr<Camera> Camera::makeOrthographic(float width, float height, float zNear, float zFar) {
    glm::mat4 projection = glm::ortho(-width / 2, width / 2, -height / 2, height / 2, zNear, zFar);
    return std::shared_ptr<Camera>(new Camera(projection));
}

std::shared_ptr<Camera> Camera::makePerspective(float fieldOfView, float aspectRatio, float zNear, float zFar) {
