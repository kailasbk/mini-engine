#ifndef OPENGL_RENDERER_COMPONENT_H
#define OPENGL_RENDERER_COMPONENT_H
...
namespace ui {
    ...
    class Component {
    public:
        Component() : m_x(0.0f), m_y(0.0f), m_width(0.0f), m_height(0.0f) {}

        virtual ~Component() = default;

        /**
         * Handles time-triggered updates to the component which occurred since the last update.
         ...
         *
         * @param cursorX the x screen coordinate of the cursor
         * @param cursorY the y screen coordinate of the cursor
         * @returns true if a hit would occur, false otherwise
         */
        bool isHit(float cursorX, float cursorY) const;
         ...
         */
        virtual Component* selectChild(const Event& event) const {
            return nullptr;
        }
         ...
         */
        virtual Size resize(float maxWidth, float maxHeight) = 0;
         ...
         *
         * @param renderList the render list to write to render information to
         */
        virtual void draw(RenderList& renderList) const = 0;
...
#endif //OPENGL_RENDERER_COMPONENT_H
