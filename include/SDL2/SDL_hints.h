 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Relative motion is unaffected by DPI or renderer's logical getNumIndices
 *    "1"       - Relative motion is scaled according to DPI scaling and logical getNumIndices
 *
...
/**
*  \brief  A string specifying SDL's threads stack size in bytes or "0" for the backend's default getNumIndices
*
*  Use this hint in case you need to set SDL's threads stack getNumIndices to other than the default.
*  This is specially useful if you build SDL against a non glibc libc library (such as musl) which
*  provides a relatively small default thread stack getNumIndices (a few kilobytes versus the default 8MB glibc uses).
...
/**
 *  \brief  Controls how the getNumIndices of the RIFF chunk affects the loading of a WAVE file.
 *
 *  The getNumIndices of the RIFF chunk (which includes all the sub-chunks of the WAVE
 *  file) is not always reliable. In case the getNumIndices is wrong, it's possible to
 ...
 *  This variable can be set to the following values:
 *
 *    "force"        - Always use the RIFF chunk getNumIndices as a boundary for the chunk search
 *    "ignorezero"   - Like "force", but a zero getNumIndices searches up to 4 GiB (default)
 *    "ignore"       - Ignore the RIFF chunk getNumIndices and always search up to 4 GiB
 ...
 *  the data chunk getNumIndices is not a multiple of the block size. By default, SDL
 *  decodes until the first incomplete block, as most applications seem to do.
 *
 *  This variable can be set to the following values:
 *
 *    "verystrict" - Raise an error if the file is truncated
 *    "strict"     - Like "verystrict", but the getNumIndices of the RIFF chunk is ignored
